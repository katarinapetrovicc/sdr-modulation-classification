...
Za prijem realnih AM i FM signala korišćen je **RTL-SDR Blog V3** SDR prijemnik sa RTL2832U čipom i R820T2 RF tjunerom.

Za proveru rada klasifikatora na realnim digitalnim signalima korišćen je javno dostupan SDR skup podataka sa poznatim oznakama modulacionih klasa.
...
Analizirani opseg je:

**118–137 MHz**

Ciljana frekvencija korišćena u eksperimentima je približno:
...
U završnim eksperimentima analiziran je signal na približno:
...
koristi se za osnovni klasifikacioni sistem pri frekvenciji uzorkovanja od **500 kS/s**.

Datoteka:

`features_final_multirate.py`

koristi se u eksperimentima sa realnim digitalnim SDR skupom pri frekvenciji uzorkovanja od **2 MS/s**.
...
Za obuku i poređenje korišćeni su sledeći algoritmi mašinskog učenja:
...
## Provera na realnim AM i FM signalima

Skripte u direktorijumu:

`classification/real_am_fm/`

koriste se za proveru klasifikacionog modela na realnim AM i FM signalima snimljenim RTL-SDR prijemnikom.
...
Preporučeni redosled pokretanja skripti je:
...
**Nikolay Belousov and Mikhail Ronkin,  
„Real-World IQ Dataset for Automatic Radio Modulation Recognition under Multipath Channels“,  
Mendeley Data, Version 2, 2026.**

**DOI:** `10.17632/tjzsbph49x.2`

Zvanična stranica skupa:

[https://data.mendeley.com/datasets/tjzsbph49x/2](https://data.mendeley.com/datasets/tjzsbph49x/2)
...
Za eksperimente korišćen je fajl:

`subset_test.h5`

Veliki `.h5` fajl nije uključen u GitHub repozitorijum.

Nakon preuzimanja skupa, datoteku `subset_test.h5` potrebno je smestiti na sledeću lokaciju:
...
Obučeni modeli čuvaju se u direktorijumu:
...
Rezultati eksperimenata čuvaju se u direktorijumu:
...
`realtime/01_realtime_fm_classification.py`
...
Skripte u repozitorijumu ne koriste apsolutne lokalne putanje poput:
